...
function createOneClient(config, app) {
  ...
  app.coreLogger.info('[egg-mongoose] connecting %s', urlMask(url, { maskUsername: false }));
  ...
  /* istanbul ignore next */
  db.on('disconnected', () => {
    app.coreLogger.error(`[egg-mongoose] ${urlMask(url, { maskUsername: false })} disconnected`);
  });

  db.on('connected', () => {
    app.coreLogger.info(`[egg-mongoose] ${urlMask(url, { maskUsername: false })} connected successfully`);
  });

  /* istanbul ignore next */
  db.on('reconnected', () => {
    app.coreLogger.info(`[egg-mongoose] ${urlMask(url, { maskUsername: false })} reconnected successfully`);
  });
